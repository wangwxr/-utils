...
if __name__=='__main__':
    device='cuda'
    source_path='../dataset/BinRushed/imaged'
    target_path='../test_checkpoint'
    os.makedirs(target_path,exist_ok=True)
    data_list=os.listdir(source_path)
    model=DEEPLABV3()
    model.cuda()
    checkpoint = torch.load('../deeplab_checkpoints/checkpoint_200_mobilenet.pth.tar')
